...
bool Application::Initialise(int _clientWidth, int _clientHeight, HINSTANCE _hInstance)
{
	...
	m_gamePadOne = new InputGamePad();
	VALIDATE(m_gamePadOne->Initialise(1));

	m_gamePadTwo = new InputGamePad();
	VALIDATE(m_gamePadTwo->Initialise(2));
...
bool Application::Initialise_DX10(HINSTANCE _hInstance)
{
	...
	pTexVec->push_back(texID);
	m_pAvatarOne = new DX10_Obj_LitTex();
	VALIDATE(m_pAvatarOne->Initialise(m_pDX10_Renderer, m_pAvatarMesh, m_pShader_LitTex, pTexVec));

	m_pDX10_Renderer->CreateTexture("WoodCrate02.dds", &texID);
	pTexVec = new std::vector < UINT >;
	pTexVec->push_back(texID);
	m_pAvatarTwo = new DX10_Obj_LitTex();
	VALIDATE(m_pAvatarTwo->Initialise(m_pDX10_Renderer, m_pAvatarMesh, m_pShader_LitTex, pTexVec));
...
void Application::ShutDown()
{
	...
	{ 
		...
		ReleasePtr(m_pAvatarOne);
		ReleasePtr(m_pAvatarTwo);
...
{
	m_gamePadOne->PreProcess();
	m_gamePadTwo->PreProcess();
   
	if (!m_gamePadOne->LStick_InDeadZone())
	{
		v2float LeftStick = m_gamePadOne->GetLStickXY();
		v3float avatarPos = m_pAvatarOne->GetPosition();

		avatarPos.x += LeftStick.x *_dt;
		avatarPos.y += LeftStick.y *_dt;

		m_pAvatarOne->SetPosition(avatarPos);
	}

	if (!m_gamePadTwo->LStick_InDeadZone())
	{
		v2float LeftStick2 = m_gamePadTwo->GetLStickXY();
		v3float avatarPos2 = m_pAvatarTwo->GetPosition();

		avatarPos2.x += LeftStick2.x *_dt;
		avatarPos2.y += LeftStick2.y *_dt;

		m_pAvatarTwo->SetPosition(avatarPos2);
	}
	m_gamePadOne->PostProcess();
	m_gamePadTwo->PostProcess();
	...
	{		
		...
		m_pAvatarOne->Process(_dt);
		m_pAvatarTwo->Process(_dt);
	}
